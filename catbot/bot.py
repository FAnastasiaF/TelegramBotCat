...
def text(message):
    if(message.text == 'Викторина на знание пород кошек'):
        ...
        bot.send_photo(message.chat.id, cat_breeds.CAT_BREEDS[message.text], reply_markup=keyboard1, disable_notification=True)
        ...
        bot.send_message(message.chat.id, 'Я такого не знаю', reply_markup=keyboard1, disable_notification=True)
